(() => {
    const e = "0.0.6";
    const CAP_WASM_JS_URL = new URL("./wasm/cap_wasm.min.js", import.meta.url).toString();
    const CAP_WASM_BG_URL = new URL("./wasm/cap_wasm_bg.wasm", import.meta.url).toString();

    if ("undefined" == typeof window) return;
    const t = (...e) => (window?.CAP_CUSTOM_FETCH ? window.CAP_CUSTOM_FETCH(...e) : fetch(...e));
    function r(e, t) {
        let r = (function (e) {
            let t = 2166136261;
            ...
            s = "";
        function i() {
            return (r ^= r << 13), (r ^= r >>> 17), (r ^= r << 5), r >>> 0;
        }
        for (; s.length < t;) {
            s += i().toString(16).padStart(8, "0");
        }
        return s.substring(0, t);
    }
    if (!window.CAP_CUSTOM_WASM_URL) {
        [CAP_WASM_JS_URL, CAP_WASM_BG_URL].forEach((e) => {
    ...
    class s extends HTMLElement {
        ...
        attributeChangedCallback(e, t, r) {
            ...
            "data-cap-worker-count" === e && this.setWorkersCount(parseInt(r, 10)),
                "data-cap-i18n-initial-state" === e &&
                this.#i &&
                this.#i?.querySelector("p")?.innerText &&
                (this.#i.querySelector("p").innerText = this.getI18nText("initial-state", "من ربات نیستم!"));
        }
        async connectedCallback() {
            ...
            this.#a.innerHTML = `<input type="hidden" name="${r}">`;
        }
        async solve() {
            if (!this.#n)
                try {
                    (this.#n = !0),
                        ...
                        this.#i.setAttribute(
                            "aria-label",
                            this.getI18nText("verifying-aria-label", "لطفاً منتظر بمانید")
                        ),
                        this.dispatchEvent("progress", { progress: 0 });
                    try {
                        let e = this.getAttribute("data-cap-api-endpoint");
                        if (!e && window?.CAP_CUSTOM_FETCH) e = "/";
                        else if (!e)
                            throw new Error(
                                "Missing API endpoint. Either custom fetch or an API endpoint must be provided."
                            );
                        e.endsWith("/") || (e += "/");
                        const { challenge: s, token: i } = await (await t(`${e}challenge`, { method: "POST" })).json();
                        let a = s;
                        if (!Array.isArray(a)) {
                            let e = 0;
                            a = Array.from(
                                { length: s.c },
                                () => ((e += 1), [r(`${i}${e}`, s.s), r(`${i}${e}d`, s.d)])
                            );
                        }
                        const n = await this.solveChallenges(a),
                            o = await (
                                await t(`${e}redeem`, {
                                    method: "POST",
                                    body: JSON.stringify({ token: i, solutions: n }),
                                    headers: { "Content-Type": "application/json" },
                                })
                            ).json();
                        if ((this.dispatchEvent("progress", { progress: 100 }), !o.success))
                            throw new Error("Invalid solution");
                        const d = this.getAttribute("data-cap-hidden-field-name") || "cap-token";
                        this.querySelector(`input[name='${d}']`) &&
                            (this.querySelector(`input[name='${d}']`).value = o.token),
                            this.dispatchEvent("solve", { token: o.token }),
                            (this.token = o.token),
                            this.#t && clearTimeout(this.#t);
                        const c = new Date(o.expires).getTime() - Date.now();
                        return (
                            c > 0 && c < 864e5
                                ? (this.#t = setTimeout(() => this.reset(), c))
                                : this.error("Invalid expiration time"),
                            this.#i.setAttribute(
                                "aria-label",
                                this.getI18nText(
                                    "verified-aria-label",
                                    "هویت شما تایید شد، ادامه دهید"
                                )
                            ),
                        ...
                        throw (
                            (this.#i.setAttribute(
                                "aria-label",
                                this.getI18nText("error-aria-label", "خطایی رخ داد، لطفاً دوباره تلاش کنید")
                            ),
                        ...
                        if (c) {
                            if (
                                ((c.onmessage = ({ data: e }) => {
                                    ...
                                    (c.onerror = (e) => {
                                        this.error(`Error in worker: ${e.message || e}`), d(e);
                                    }),
                                    c.postMessage({
                                        salt: t,
                                        target: a,
        ...
        setWorkersCount(e) {
            ...
            this.#r = !Number.isNaN(t) && t > 0 && t <= r ? t : navigator.hardwareConcurrency || 8;
        }
        createUI() {
                ...
                (this.#i.querySelector("a").addEventListener("click", (e) => {
                    ...
                    this.#i.addEventListener("keydown", (e) => {
                        ("Enter" !== e.key && " " !== e.key) ||
                            ...
                            (e.preventDefault(), e.stopPropagation(), this.solve());
                    }),
        ...
        handleProgress(e) {
            ...
            const t = this.#i.querySelector("p"),
                r = this.#i.querySelector(".progress-ring-circle");
            if (t && r) {
                const s = 2 * Math.PI * 14,
                    i = s - (e.detail.progress / 100) * s;
                (r.style.strokeDashoffset = i),
                    (t.innerText = `${this.getI18nText("verifying-label", "در حال بررسی...")} ${e.detail.progress}%`);
            }
            this.executeAttributeCode("onprogress", e);
        }
        handleSolve(e) {
            this.updateUI("done", this.getI18nText("solved-label", "تایید شد!"), !0),
        ...
        executeAttributeCode(e, t) {
            const r = this.getAttribute(e);
            r && new Function("event", r).call(this, t);
        }
        error(e = "Unknown error") {
            console.error("[cap]", e), this.dispatchEvent("error", { isCap: !0, message: e });
        }
        dispatchEvent(e, t = {}) {
    ...
    class i {
        constructor(e = {}, t) {
            const r = t || document.createElement("cap-widget");
            if (
                (Object.entries(e).forEach(([e, t]) => {
                    r.setAttribute(e, t);
                }),
                    !e.apiEndpoint && !window?.CAP_CUSTOM_FETCH)
            )
                throw (
                    (r.remove(),
                        new Error("Missing API endpoint. Either custom fetch or an API endpoint must be provided."))
                );
            e.apiEndpoint && r.setAttribute("data-cap-api-endpoint", e.apiEndpoint),
                (this.widget = r),
                ...
                Object.defineProperty(this, "token", { get: () => r.token, configurable: !0, enumerable: !0 }),
                t || ((r.style.display = "none"), document.documentElement.appendChild(r));
        }
    }
    (window.Cap = i),
        customElements.get("cap-widget") || window?.CAP_DONT_SKIP_REDEFINE
            ? console.warn(
